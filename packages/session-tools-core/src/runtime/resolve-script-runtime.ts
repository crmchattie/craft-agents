...
export interface ResolveScriptRuntimeContext {
  /**
   * Whether host app is packaged. Defaults to SCRUNCHY_IS_PACKAGED=1.
...
function inferPackagedMode(ctx?: ResolveScriptRuntimeContext): boolean {
  ...
  return process.env.SCRUNCHY_IS_PACKAGED === '1';
}
...
function resolveResourcesBase(ctx?: ResolveScriptRuntimeContext): string | null {
  const explicit = ctx?.resourcesBasePath || process.env.SCRUNCHY_RESOURCES_BASE;
...
function resolveAppRoot(ctx?: ResolveScriptRuntimeContext): string | null {
  const explicit = ctx?.appRootPath || process.env.SCRUNCHY_APP_ROOT;
...
function validatePackagedEnvRuntime(command: string, label: string): string {
  ...
  if (!isAbsolute(command) && !hasPathSeparator) {
    throw new Error(
      ...
      'Packaged builds do not allow PATH-based runtime resolution. Configure an absolute SCRUNCHY_* path or ship a bundled runtime.'
    );
  }
  ...
  if (!existsSync(resolvedCommand)) {
    throw new Error(
      ...
      'Configure a valid absolute SCRUNCHY_* path or ship a bundled runtime.'
    );
  }
 ...
 *
 * Resolution order:
 * - env override (SCRUNCHY_UV / SCRUNCHY_NODE / SCRUNCHY_BUN)
...
export function resolveScriptRuntime(
  ...
  if (language === 'python3') {
    if (process.env.SCRUNCHY_UV) {
      const cmd = isPackaged
        ? validatePackagedEnvRuntime(process.env.SCRUNCHY_UV, 'Python/uv')
        : process.env.SCRUNCHY_UV;
    ...
    throw new Error(
      isPackaged
        ...
        : 'Python runtime unavailable: uv was not found. Configure SCRUNCHY_UV or install uv on PATH.'
    );
  }

  if (language === 'node') {
    if (process.env.SCRUNCHY_NODE) {
      const cmd = isPackaged
        ? validatePackagedEnvRuntime(process.env.SCRUNCHY_NODE, 'Node')
        : process.env.SCRUNCHY_NODE;
    ...
    throw new Error(
      isPackaged
        ...
        : 'Node runtime unavailable: configure SCRUNCHY_NODE or install node on PATH.'
    );
  }

  if (process.env.SCRUNCHY_BUN) {
    const cmd = isPackaged
      ? validatePackagedEnvRuntime(process.env.SCRUNCHY_BUN, 'Bun')
      : process.env.SCRUNCHY_BUN;
    return { command: cmd, argsPrefix: [], source: 'env' };
  }
  ...
  throw new Error(
    isPackaged
      ...
      : 'Bun runtime unavailable: configure SCRUNCHY_BUN or install bun on PATH.'
  );
}
