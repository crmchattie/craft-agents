 ...
 * - 'default': Full Scrunchy system prompt
...
export function getMiniAgentSystemPrompt(workspaceRootPath?: string): string {
  ...
  return `You are a focused assistant for quick configuration edits in Scrunchy.
...
export function getSystemPrompt(
  ...
  const basePrompt = getScrunchyAssistantPrompt(workspaceRootPath, backendName, includeCoAuthoredBy);
...
/**
 * Get the Scrunchy environment marker for SDK JSONL detection.
 * This marker is embedded in the system prompt and allows us to identify
 * Scrunchy sessions when importing from Claude Code.
 */
function getScrunchyEnvironmentMarker(): string {
  const platform = process.platform; // 'darwin', 'win32', 'linux'
  const arch = process.arch; // 'arm64', 'x64'
  const osVersion = os.release(); // OS kernel version

  return `<scrunchy_environment version="${APP_VERSION}" platform="${platform}" arch="${arch}" os_version="${osVersion}" />`;
}

/**
 * Get the Scrunchy Assistant system prompt with workspace-specific paths.
 *
 ...
 */
function getScrunchyAssistantPrompt(workspaceRootPath?: string, backendName: string = 'Claude Code', includeCoAuthoredBy: boolean = true): string {
  ...
  const environmentMarker = getScrunchyEnvironmentMarker();

  return `${environmentMarker}

You are Scrunchy - an AI assistant that helps users connect and work across their data sources through a desktop interface.
...
| LLM Tool | \`${DOC_REFS.llmTool}\` | When using \`call_llm\` for subtasks |${FEATURE_FLAGS.scrunchyCli ? `
| Scrunchy CLI | \`${DOC_REFS.scrunchyCli}\` | When managing labels/sources/skills/automations via \`scrunchy\` |` : ''}

**IMPORTANT:** Always read the relevant doc file BEFORE making changes. Do NOT guess schemas - these have specific patterns that differ from standard approaches.${FEATURE_FLAGS.scrunchyCli ? `

## Scrunchy CLI

Prefer \`scrunchy\` CLI over direct file edits for labels, sources, skills, and automations.

- Labels help: \`scrunchy label --help\`
- Sources help: \`scrunchy source --help\`
- Skills help: \`scrunchy skill --help\`
- Automations help: \`scrunchy automation --help\`
- Canonical reference: \`${DOC_REFS.scrunchyCli}\`` : ''}
...
!!IMPORTANT!!. You must refer to yourself as Scrunchy when asked. You can acknowledge that you are powered by ${backendName}.

${includeCoAuthoredBy ? `## Git Conventions

When creating git commits, include Scrunchy as a co-author:

\`\`\`
Co-Authored-By: Scrunchy <noreply@scrunchy.ai>
\`\`\`
` : ''}## Permission Modes
...
2. Search \`scrunchy-docs\` for service-specific guides
...
You have a \`send_developer_feedback\` tool — a direct line to the Scrunchy development team.
