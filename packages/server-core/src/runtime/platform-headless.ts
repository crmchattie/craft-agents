...
function createConsoleLogger(): Logger {
  ...
  return {
    ...
    debug: (...args) => {
      if (process.env.SCRUNCHY_DEBUG === 'true' || process.env.SCRUNCHY_IS_PACKAGED !== 'true') {
 ...
 * - SCRUNCHY_APP_ROOT — override appRootPath (default: cwd)
 * - SCRUNCHY_RESOURCES_PATH — override resourcesPath (default: cwd/resources)
 * - SCRUNCHY_IS_PACKAGED — 'true' for production (default: false)
 * - SCRUNCHY_VERSION — app version string (default: '0.0.0-dev')
 * - SCRUNCHY_DEBUG — 'true' to enable debug logging
 */
export function createHeadlessPlatform(options?: { appVersion?: string }): PlatformServices {
  const logger = createConsoleLogger()
  const isDebugMode = process.env.SCRUNCHY_DEBUG === 'true' || process.env.SCRUNCHY_IS_PACKAGED !== 'true'

  return {
    appRootPath: process.env.SCRUNCHY_APP_ROOT || process.cwd(),
    resourcesPath: process.env.SCRUNCHY_RESOURCES_PATH || join(process.cwd(), 'resources'),
    isPackaged: process.env.SCRUNCHY_IS_PACKAGED === 'true',
    appVersion: process.env.SCRUNCHY_VERSION || options?.appVersion || '0.0.0-dev',
