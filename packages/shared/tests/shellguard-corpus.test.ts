...
const TEST_MODE_CONFIG = {
  ...
  readOnlyBashPatterns: [
    ...
    // scrunchy CLI read-only
    { regex: /^scrunchy\s+label\s+(list|get)\b/, source: '^scrunchy\\s+label\\s+(list|get)\\b', comment: 'scrunchy label read-only operations' },
    { regex: /^scrunchy\s+source\s+(list|get|validate|test)\b/, source: '^scrunchy\\s+source\\s+(list|get|validate|test)\\b', comment: 'scrunchy source read-only operations' },
    { regex: /^scrunchy\s+skill\s+(list|get|validate|where)\b/, source: '^scrunchy\\s+skill\\s+(list|get|validate|where)\\b', comment: 'scrunchy skill read-only operations' },
    { regex: /^scrunchy\s+automation\s+(list|get|validate|history|last-executed|test|lint)\b/, source: '^scrunchy\\s+automation\\s+(list|get|validate|history|last-executed|test|lint)\\b', comment: 'scrunchy automation read-only operations' },
    { regex: /^scrunchy\s*$/, source: '^scrunchy\\s*$', comment: 'scrunchy bare invocation' },
    { regex: /^scrunchy\s+(label|source|skill|automation)\s+--help\b/, source: '^scrunchy\\s+(label|source|skill|automation)\\s+--help\\b', comment: 'scrunchy entity help flags' },
    { regex: /^scrunchy\s+--(help|version|discover)\b/, source: '^scrunchy\\s+--(help|version|discover)\\b', comment: 'scrunchy global flags' },
...
// ============================================================
// Group 27: scrunchy CLI allowlist
// ============================================================

describe('ShellGuard corpus: scrunchy CLI allowlist', () => {
  const shouldAllow = [
    'scrunchy',
    'scrunchy --help',
    'scrunchy --version',
    ...
    'scrunchy label --help',
    'scrunchy label list',
    'scrunchy label get bug',
    'scrunchy source --help',
    'scrunchy source list',
    'scrunchy source get linear',
    'scrunchy source validate linear',
    'scrunchy source test linear',
    'scrunchy skill --help',
    'scrunchy skill list',
    'scrunchy skill get commit-helper',
    'scrunchy skill where commit-helper',
    'scrunchy skill validate commit-helper',
    'scrunchy automation list',
    'scrunchy automation get abc123',
    'scrunchy automation validate',
    'scrunchy automation history abc123 --limit 5',
    'scrunchy automation last-executed abc123',
    'scrunchy automation test abc123 --match "x"',
    'scrunchy automation lint',
  ];

  const shouldBlock = [
    'scrunchy label create --name Bug',
    'scrunchy label update bug --name "Bug Report"',
    'scrunchy label delete bug',
    'scrunchy label move bug --parent root',
    'scrunchy label reorder --parent root a b c',
    'scrunchy source create --name Linear --provider linear --type mcp',
    'scrunchy source update linear --json "{\"enabled\":false}"',
    'scrunchy source delete linear',
    'scrunchy skill create --name "Review" --description "x"',
    'scrunchy skill update review --json "{\"description\":\"y\"}"',
    'scrunchy skill delete review',
    'scrunchy automation create --event UserPromptSubmit --prompt "x"',
    'scrunchy automation update abc123 --json "{\"enabled\":false}"',
    'scrunchy automation delete abc123',
    'scrunchy automation enable abc123',
    'scrunchy automation disable abc123',
    'scrunchy automation duplicate abc123',
  ];
