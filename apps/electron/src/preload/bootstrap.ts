 ...
 *
 * Thin-client mode (SCRUNCHY_SERVER_URL):
...
import { createCallbackServer } from '@scrunchy/shared/auth/callback-server'
import { CHATGPT_OAUTH_CONFIG } from '@scrunchy/shared/auth/chatgpt-oauth-config'
import {
...
import type { ConfirmDialogSpec, FileDialogSpec } from '@scrunchy/server-core/transport'
import type { RpcClient } from '@scrunchy/server-core/transport'
import type { RemoteServerConfig } from '@scrunchy/core/types'
...
if (isClientOnly) {
  ...
  const wsUrl = process.env.SCRUNCHY_SERVER_URL!
  const wsToken = process.env.SCRUNCHY_SERVER_TOKEN ?? ''
  ...
  if (parsed.protocol === 'ws:' && !isLocalhost) {
    throw new Error(
      ...
      `Set SCRUNCHY_RPC_TLS_CERT/KEY on the server to enable TLS.`
    )
  }

  // Workspace ID is optional — if missing, renderer shows a workspace picker
  const workspaceId = process.env.SCRUNCHY_WORKSPACE_ID || ipcRenderer.sendSync('__get-workspace-id') || undefined
...
;(api as ElectronAPI).getSystemWarnings = async () => ({
  vcredistMissing: process.env.SCRUNCHY_VCREDIST_MISSING === '1',
  downloadUrl: process.env.SCRUNCHY_VCREDIST_URL,
})
