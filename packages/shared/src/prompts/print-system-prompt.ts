#!/usr/bin/env bun
/**
 * Debug script to print the full Scrunchy system prompt with annotations.
...
${colors.bgMagenta}${colors.bold}                    SCRUNCHY SYSTEM PROMPT BREAKDOWN                         ${colors.reset}
${colors.bgMagenta}${colors.bold}                                                                                ${colors.reset}
`);
...
printAnnotation('  2. Scrunchy Environment Marker - version, platform, arch');
...
const systemPrompt = getSystemPrompt(
  ...
  '/Users/example/.scrunchy/workspaces/abc123' // Example workspace path
);
...
const systemPromptWithDebug = getSystemPrompt(
  undefined,
  { enabled: true, logFilePath: '~/Library/Logs/@scrunchy/electron/main.log' },
  '/Users/example/.scrunchy/workspaces/abc123'
);
...
const sessionState = formatSessionState('260121-example-session', {
  plansFolderPath:
    '/Users/example/.scrunchy/workspaces/abc123/sessions/260121-example-session/plans',
});
...
printAnnotation('Generated by Scrunchy.formatSourceState() - requires agent instance');
...
${colors.bold}SDK Configuration:${colors.reset}
  ...
  systemPrompt.append: getSystemPrompt() ${colors.dim}// Scrunchy additions (static, cacheable)${colors.reset}
...
${colors.bold}Static System Prompt Components:${colors.reset}
  ...
  2. Scrunchy Environment Marker      ${colors.dim}// Version, platform, arch${colors.reset}
...
${colors.bold}Key Files:${colors.reset}
  ...
  packages/shared/src/agent/scrunchy.ts       ${colors.dim}// User message building${colors.reset}
