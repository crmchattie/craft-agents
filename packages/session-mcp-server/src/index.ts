 ...
 * It uses the shared handlers from @scrunchy/session-tools-core to ensure
 ...
 *   --workspace-root: Path to workspace folder (~/.scrunchy/workspaces/{id})
...
import { isDeveloperFeedbackEnabled } from '@scrunchy/shared/feature-flags';
...
function createCodexContext(config: SessionConfig): SessionToolContext {
  ...
  return {
    ...
    updatePreferences: (updates: Record<string, unknown>) => {
      ...
      // workspaceRootPath = ~/.scrunchy/workspaces/{id}
      // preferencesPath = ~/.scrunchy/preferences.json
    ...
    submitFeedback: (feedback) => {
      const configDir = process.env.SCRUNCHY_CONFIG_DIR || join(workspaceRootPath, '..', '..');
...
/**
 * Connect to the scrunchy-docs MCP server and fetch its tool definitions.
...
async function connectDocsUpstream(): Promise<void> {
  try {
    const client = new Client(
      { name: 'scrunchy-session-proxy', version: '1.0.0' },
      { capabilities: {} }
    );
    ...
    console.error(`Scrunchy Docs proxy connected: ${docsTools.length} tools`);
  } catch (err) {
    console.error(`Scrunchy Docs proxy connection failed (non-fatal): ${err instanceof Error ? err.message : String(err)}`);
    docsClient = null;
    docsTools = [];
  }
}
...
async function callDocsUpstream(
  ...
  if (!docsClient) {
    return errorResponse(`Scrunchy Docs server is not connected. Tool '${name}' unavailable.`);
  }
...
async function handleCallLlm(
  ...
  // Uses callbackPort from CLI arg (--callback-port) or env var (SCRUNCHY_LLM_CALLBACK_PORT).
  ...
  return errorResponse(
    ...
    'HTTP callback (SCRUNCHY_LLM_CALLBACK_PORT). Neither is available.'
  );
}
...
async function main() {
  ...
  const config: SessionConfig = {
    ...
    callbackPort: callbackPort || process.env.SCRUNCHY_LLM_CALLBACK_PORT,
  };
