...
// Plan types are used by UI components; not needed in scrunchy.ts since Safe Mode is user-controlled
...
// Documentation is served via local files at ~/.scrunchy/docs/

// Import and re-export AgentEvent from core (single source of truth)
import type { AgentEvent } from '@scrunchy/core/types';
  ...
  protected async *chatImpl(
    ...
    try {
      ...
      const fullMcpServers: Options['mcpServers'] = {
        ...
        // Documentation - always available for searching setup guides
        // Using Craft Agents public Mintlify MCP server, no auth needed
        'scrunchy-docs': {
          type: 'http',
      ...
      // Gated by enable1MContext in global config (~/.scrunchy/config.json).
      ...
      const options: Options = {
        ...
        hooks: (() => {
          ...
          if (Object.keys(userHooks).length > 0) {
            debug('[Scrunchy] User SDK hooks loaded:', Object.keys(userHooks).join(', '));
          }
...
// ============================================================
// These aliases allow gradual migration from Scrunchy to ClaudeAgent.
// Once all consumers are updated, these can be removed.

/** @deprecated Use ClaudeAgent instead */
export { ClaudeAgent as Scrunchy };

/** @deprecated Use ClaudeAgentConfig instead */
export type { ClaudeAgentConfig as ScrunchyConfig };
