 ...
 * Deep Link Handler
 *
 * Parses scrunchy:// URLs and routes to appropriate actions.
 *
 * URL Formats (workspace is optional - uses active window if omitted):
 *
 * Compound format (hierarchical navigation):
 *   scrunchy://allSessions[/session/{sessionId}]            - Session list (all sessions)
 *   scrunchy://flagged[/session/{sessionId}]             - Session list (flagged filter)
 *   scrunchy://state/{stateId}[/session/{sessionId}]     - Session list (state filter)
 *   scrunchy://sources[/source/{sourceSlug}]          - Sources list
 *   scrunchy://settings[/{subpage}]                   - Settings (general, shortcuts, preferences)
 *
 * Action format:
 *   scrunchy://action/{actionName}[/{id}][?params]
 *   scrunchy://workspace/{workspaceId}/action/{actionName}[?params]
 *
 * Actions:
 ...
 *
 * Examples:
 *   scrunchy://allSessions                               (all sessions view)
 *   scrunchy://allSessions/session/abc123                (specific session)
 *   scrunchy://settings/shortcuts                     (shortcuts page)
 *   scrunchy://sources/source/github                  (github source info)
 *   scrunchy://action/new-chat                        (uses active window)
 *   scrunchy://action/resume-sdk-session/{sdkId}      (resume Claude Code session)
 *   scrunchy://workspace/ws123/allSessions/session/abc123   (targets specific workspace)
 */
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    // e.g., scrunchy://workspace/ws123 → hostname='workspace', pathname='/ws123'
    // e.g., scrunchy://allSessions/chat/abc → hostname='allSessions', pathname='/chat/abc'
    const host = parsed.hostname
    const pathParts = parsed.pathname.split('/').filter(Boolean)
    const windowMode = parseWindowMode(parsed)
    const rightSidebar = parseRightSidebar(parsed)

    // scrunchy://auth-callback?... (OAuth callbacks - return null to let existing handler process)
    ...
    // scrunchy://allSessions/..., scrunchy://settings/..., etc. (compound routes)
    ...
    // scrunchy://workspace/{workspaceId}/... (with workspace targeting)
    if (host === 'workspace') {
    ...
    // scrunchy://action/... (no workspace - uses active window)
    if (host === 'action') {
