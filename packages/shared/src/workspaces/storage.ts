 ...
 * Default location: ~/.scrunchy/workspaces/
 */
...
const CONFIG_DIR = join(homedir(), '.scrunchy');
...
/**
 * Get the default workspaces directory (~/.scrunchy/workspaces/)
 */
 ...
 * @param baseDir - Parent directory where workspace folders live (e.g., ~/.scrunchy/workspaces/)
 ...
 * Returns paths to valid workspaces found in ~/.scrunchy/workspaces/
 */
 ...
 * Resolution order: ENV (SCRUNCHY_LOCAL_MCP_ENABLED) > workspace config > default (true)
 *
 * @param rootPath - Absolute path to workspace root folder
...
export function isLocalMcpEnabled(rootPath: string): boolean {
  ...
  const envValue = process.env.SCRUNCHY_LOCAL_MCP_ENABLED;
...
export function ensurePluginManifest(rootPath: string, workspaceName: string): void {
  ...
  const manifest = {
    name: `scrunchy-workspace-${workspaceName.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`,
    version: '1.0.0',
  };
