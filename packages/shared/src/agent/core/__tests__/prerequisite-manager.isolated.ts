...
function browserDocPath(): string {
  return resolve(join(homedir(), '.scrunchy', 'docs', 'browser-tools.md'));
}
...
describe('PrerequisiteManager', () => {
  ...
  describe('rule matching', () => {
    ...
    it('exempts scrunchy-docs MCP tools', () => {
      mockExistsPaths.add(guidePath('scrunchy-docs'));
      const result = manager.checkPrerequisites('mcp__scrunchy-docs__search');
      expect(result.allowed).toBe(true);
    });
