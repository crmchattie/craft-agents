...
import { getSessionSafeAllowedToolNames } from '@scrunchy/session-tools-core';
...
class ModeManager {
  ...
  setPermissionMode(
    ...
    // Notify callbacks (for Scrunchy internal sync)
...
export function getPathHint(targetPath: string, plansFolderPath: string, dataFolderPath?: string): string | null {
  ...
  if (normalizedTarget.includes('/.scrunchy/workspaces/') && !normalizedTarget.includes('/sessions/')) {
    return 'Hint: Write to the session plans or data folder, not the workspace root.';
  }

  // Case: Writing outside .scrunchy entirely
  if (!normalizedTarget.includes('/.scrunchy/')) {
...
export function shouldAllowToolInMode(
  ...
  if (toolName.startsWith('mcp__')) {
    ...
    if (toolName.startsWith('mcp__scrunchy-docs__')) {
      return { allowed: true };
    }
