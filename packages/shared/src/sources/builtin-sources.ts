 ...
 *
 * NOTE: scrunchy-docs is now an always-available MCP server configured
 * directly in scrunchy.ts, not a source. This file is kept for backwards
 ...
 *
 * Currently returns empty array - scrunchy-docs has been moved to
 * an always-available MCP server in scrunchy.ts.
 *
...
/**
 * Get the built-in Scrunchy docs source.
 *
 * @deprecated scrunchy-docs is now an always-available MCP server
 * configured directly in scrunchy.ts. This function is kept for
...
export function getDocsSource(workspaceId: string, workspaceRootPath: string): LoadedSource {
  ...
  const placeholderConfig: FolderSourceConfig = {
    ...
    mcp: {
      transport: 'http',
      url: 'https://agents.craft.do/docs/mcp', // Using Craft Agents public docs
      authType: 'none',
    },
    tagline: 'Search Scrunchy documentation and source setup guides',
    icon: '📚',
 ...
 *
 * Returns false - scrunchy-docs is now an always-available MCP server,
