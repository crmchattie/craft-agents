 ...
 * @param workspaceRootPath - Absolute path to workspace folder (e.g., ~/.scrunchy/workspaces/xxx)
 ...
 * (like scrunchy-docs) that don't have filesystem folders.
 */
export function getSourcesBySlugs(workspaceRootPath: string, slugs: string[]): LoadedSource[] {
  ...
  for (const slug of slugs) {
    ...
    if (isBuiltinSource(slug)) {
      // Currently only scrunchy-docs is a builtin source
      if (slug === 'scrunchy-docs') {
 ...
 * Built-in sources (like scrunchy-docs) are always available and merged
...
export async function createSource(
  ...
  // (bundled guides removed - agent should search scrunchy-docs MCP for service-specific guidance)
