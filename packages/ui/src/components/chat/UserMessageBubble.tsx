...
import type { StoredAttachment, ContentBadge } from '@scrunchy/core'
import { normalizePath } from '@scrunchy/core/utils'
...
function InlineFileBadge({
  ...
  // Strip .scrunchy workspace/session path prefix for cleaner tooltip display
  // e.g. "/Users/.../workspaces/{id}/sessions/{id}/plans/foo.md" → "plans/foo.md"
  const rawPath = badge.filePath || badge.label
  const tooltipPath = normalizePath(rawPath).replace(/^.*\.scrunchy\/workspaces\/[^/]+\/(sessions\/[^/]+\/)?/, '')
