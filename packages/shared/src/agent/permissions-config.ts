 ...
 *
 * File locations:
 * - Workspace: ~/.scrunchy/workspaces/{slug}/permissions.json
 * - Per-source: ~/.scrunchy/workspaces/{slug}/sources/{sourceSlug}/permissions.json
 *
 ...
 * Default permissions are stored at ~/.scrunchy/permissions/
 * Reads env var dynamically so tests can override via SCRUNCHY_CONFIG_DIR.
 */
export function getAppPermissionsDir(): string {
  const configDir = process.env.SCRUNCHY_CONFIG_DIR || join(homedir(), '.scrunchy');
...
/**
 * Load default permissions from ~/.scrunchy/permissions/default.json
...
function shouldCompileBashPattern(pattern: string): boolean {
  if (!FEATURE_FLAGS.scrunchyCli && pattern.startsWith('^scrunchy\\s')) {
    return false;
  }
  return true;
}
...
class PermissionsConfigCache {
  ...
  // App-level default permissions (loaded from ~/.scrunchy/permissions/default.json)
   ...
   * These come from ~/.scrunchy/permissions/default.json
   */
  ...
  private applyDefaultConfig(merged: MergedPermissionsConfig, config: PermissionsCustomConfig): void {
    ...
    for (const patternEntry of config.allowedBashPatterns) {
      if (!shouldCompileBashPattern(patternEntry.pattern)) {
        debug(`[Permissions] Skipping scrunchy bash pattern (feature disabled): ${patternEntry.pattern}`);
        continue;
      }
